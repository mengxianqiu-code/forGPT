...
document.getElementById("analyzeBtn").addEventListener("click", async () => {
  try {
    ...
    const llmText = data.llm_summary ? `\nQwen洞察：${data.llm_summary}` : "";
    const llmError = data.llm_error ? `\n提示：${data.llm_error}` : "";
    summaryEl.textContent = `共分析 ${data.summary.post_count} 篇帖子，${data.summary.keyword_count} 个关键词，总命中 ${data.summary.total_mentions} 次。${llmText}${llmError}`;
